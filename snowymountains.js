function draw(){
    ...
    i=1;
    do{
        snowflakes.push([random_number(width),0,flake_size,flake_size,random_number(4)])
    }while(i--);
    ...
    do{
        if(snowflakes[i][1]>height){
            ...
            /*draw snowflake*/
            canvas.fillRect(snowflakes[i][0],snowflakes[i][1],snowflakes[i][2],snowflakes[i][3])
        }
    }while(i--)
}
...
function update_background(){
    a=y*.75;
    k=y*.35;
    var trees=[],j=0,math=[0,0,0,0,0,0,0];
    ...
    do{
        ...
        trees.push([Math.random(),j,-(y/2-j)/k,'#'+random_number(5)+(random_number(5)+4)+random_number(5)])
    }while(i--);
    ...
    /*precalculate stuff*/
    math=[width/100,width/45,x*.4,y*.4,x*1.6,height/100,x*.9,x*.7];
    ...
    do{
        buffer.fillRect(width*trees[i][0]-math[0]*2,trees[i][1],math[0]*trees[i][2],math[0]*trees[i][2])
    }while(i--);
    ...
    i=1;
    do{
        buffer.fillStyle=[trees[0][3],'#0d0'][i];
        buffer.beginPath();
        buffer.arc(width*trees[0][0],trees[0][1]-(height/7),height/(40-20*i),0,Math.PI*2,false);
        buffer.closePath();
        buffer.fill()
    }while(i--);
    ...
    buffer.beginPath();
    do{
        buffer.arc(width*trees[0][0]+math[5]*([-3.5,-2.5,0,2.5,3.5,2.5,0,-2.5][i]),trees[0][1]-(height/7)+math[5]*([0,-2.5,-3.5,-2.5,0,2.5,3.5,2.5][i]),math[5],0,Math.PI*2,false)
    }while(i--);
    buffer.closePath();
    buffer.fill();
    ...
    buffer.fillText('Veselé Vánoce!',5,47)
}
var a=buffer=canvas=flake_size=height=i=k=width=x=y=0,
snowflakes=[];
